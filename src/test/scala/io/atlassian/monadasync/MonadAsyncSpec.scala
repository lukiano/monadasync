...
object MonadAsyncProperties {
  ...
  object monadAsync {
    def asyncIsSuspend[F[_], A](implicit ma: MonadAsync[F], eq: Equal[F[A]], a0: Arbitrary[() => A]) =
      forAll(ma.monadAsyncLaw.asyncIsSuspend[A] _)
    ...
    def laws[M[_]](implicit ma: MonadAsync[M], am: Arbitrary[M[Int]], a: Arbitrary[Int], am0: Arbitrary[() => Int],
      ...
      new Properties("monad async") {
        property("asyncIsSuspend") = asyncIsSuspend[M, Int]
