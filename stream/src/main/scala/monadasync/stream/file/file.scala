...
package object file {
  ...
  def safe[F[_]: MonadSuspend: Monad: Catchable](is: => InputStream): Process[F, ByteVector] =
    Process.constant(bufferSize).asInstanceOf[Process[F, Int]].through[F, ByteVector](chunkR[F](is))

  def chunkR[F[_]: MonadSuspend: Monad: Catchable](is: => InputStream): Channel[F, Int, ByteVector] =
    unsafeChunkR(is).map(f => (n: Int) =>
      f(new Array[Byte](n)) map ByteVector.view)

  private def unsafeChunkR[F[_]: MonadSuspend: Monad: Catchable](is: => InputStream): Channel[F, Array[Byte], Array[Byte]] = {
    resource(attempt(is))(
  ...
  def decodeMmap[F[_]: MonadAsync: Monad: Catchable, A](sc: StreamCodec[A], in: => FileChannel, chunkSizeInBytes: Int = 1024 * 1000 * 16): Process[F, A] = {
  ...
  private def attempt[F[_]: MonadSuspend: Monad: Catchable, A](a: => A): F[A] =
    MonadSuspend[F].suspend(tryCatch[F, A](a))
  ...
  def asyncChunkW[F[_]: MonadAsync: Monad: Catchable](f: Path, append: Boolean = false): Sink[F, ByteVector] = {
  ...
  def fileChunkW[F[_]: MonadSuspend: Monad: Catchable](f: Path, append: Boolean = false): Sink[F, ByteVector] =
    resource(
      attempt {
